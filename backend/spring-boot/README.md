...
### Основы слоев
1.  [**Controller (Контроллер)**](./Controller.md) — Входная точка приложения, обработка запросов.
2.  [**Service (Сервис)**](./Service.md) — Бизнес-логика и координация.
3.  [**Repository (Репозиторий)**](./Repository.md) — Работа с базой данных (CRUD).
4.  [**Model**](./Model.md) — Сущности базы данных (Entities).
5.  [**DTO**](./DTO.md) — Объекты передачи данных.

### Инструменты и механизмы
*   [**Validation**](./Validation.md) — Проверка входящих данных.
*   [**Lombok**](./Lombok.md) — Генерация шаблонного кода (Getters/Setters).
*   [**Security**](../security/Security.md) — Аутентификация и авторизация.
*   [**Testing**](../../qa-and-testing/unit-testing/Testing.md) — Unit и интеграционные тесты.
*   [**Logging**](./Logging.md) — Как настроить логи (Slf4j).
*   [**Swagger / OpenAPI**](./Swagger.md) — Документация API (`@Operation`, `@Tag`).
*   [**Thymeleaf**](./Thymeleaf.md) — Шаблонизатор для генерации HTML на сервере.
*   [**File Upload**](./FileUpload.md) — Загрузка и отдача файлов (MultipartFile).
*   [**E-mail**](./Email.md) — Отправка почты (SMTP, JavaMailSender).
*   [**Docker**](../../devops-and-tools/docker/Docker.md) — Упаковка приложения и БД в контейнеры.
*   [**Maven**](./Maven.md) — Система сборки и управления зависимостями (`pom.xml`).
*   [**Gradle**](./Gradle.md) — Мощная и гибкая система сборки проекта (`build.gradle`).
*   [**HTTP Statuses**](./HttpStatuses.md) — Справочник кодов ответов (200, 401, 403 и др.).
*   [**HTTP Methods**](./HttpMethods.md) — Подробно о GET, POST, PUT, DELETE и идемпотентности.
*   [**Exception Handling**](./ExceptionHandling.md) — Глобальная обработка ошибок (`@ControllerAdvice`).
*   [**Configuration**](./Configuration.md) — Настройки, профили, `application.yml` и `@Value`.
*   [**Pagination**](./Pagination.md) — Пагинация и сортировка данных (`Pageable`).
*   [**Migrations**](./Migrations.md) — Управление структурой БД, отказ от `ddl-auto=update` (Flyway / Liquibase).
*   [**MapStruct**](./MapStruct.md) — Автоматический маппинг `DTO` <-> `Entity`.
*   [**Utils**](./Utils.md) — Вспомогательные классы.
